...
export default function SystemPage() {
  ...
  const loadAllData = async () => {
    ...
    try {
      ...
      await Promise.all([loadDevices(), loadZones(), loadApiKeys()]);
    } catch (err) {
  ...
  const handleAddDevice = async () => {
    try {
      ...
      showSuccessMessage(
        'Device added successfully! Check the API Keys section below for your device configuration.',
      );
    } catch (err) {
  ...
  const handleCreateZone = async () => {
    try {
      ...
      await loadZones();
      showSuccessMessage(
        'Zone created successfully! You can now select it from the dropdown.',
      );
    } catch (err) {
  ...
  const handleCreateTestDevice = async () => {
    try {
      ...
      await loadAllData();
      showSuccessMessage(
        'Test device created successfully! Check the API Keys section below.',
      );
    } catch (err) {
      console.error('Error creating test device:', err);
      setError(
        'Failed to create test device: ' + (err.message || 'Unknown error'),
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleGenerateApiKeyForDevice = async (deviceId) => {
    try {
      ...
      await loadApiKeys();
      showSuccessMessage(
        'API key generated successfully! Check the API Keys section below.',
      );
    } catch (err) {
  ...
  const handleRefreshSchema = async () => {
    try {
      ...
      if (result.success) {
        await loadAllData();
        showSuccessMessage(
          'Schema cache refreshed successfully! Try creating a device now.',
        );
      } else {
        setError(
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                API keys and configuration data for your ESP32 devices
              </p>
            </div>
            <button
              onClick={loadApiKeys}
          ...
          {apiKeys.length > 0 ? (
            <div className="space-y-4">
              {apiKeys.map((apiKey) => (
                <div
                  key={apiKey.id}
                  className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 sm:p-6 transition-all duration-200 hover:shadow-md"
                >
                    ...
                    <div>
                      ...
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                        <div className="flex-1 relative">
                          <input
                            type={
                              hiddenApiKeys.has(apiKey.id) ? 'password' : 'text'
                            }
                            value={apiKey.key}
                            readOnly
            ...
            <div className="text-center py-8">
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500">
                Add a device to generate API keys for ESP32 configuration
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                ESP32 devices registered to your account
              </p>
            </div>
            <button
              onClick={loadDevices}
                ...
                >
                  ...
                  <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600 space-y-2">
                    {hasApiKeyForDevice(device.name) ? (
                      ...
                      <button
                        onClick={() =>
                          handleGenerateApiKeyForDevice(device.device_id)
                        }
                        disabled={loading}
                    ...
                    <div className="flex space-x-2">
                      ...
                      <button
                        onClick={() =>
                          confirmDeleteDevice(device.id, device.name)
                        }
                        disabled={loading}
